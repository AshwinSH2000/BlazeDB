...
public class BlazeDB {
	 ...
	 *
	 * @param args The command line input that contains 1) path for data & schema,
	...
	public static void main(String[] args) {
		...
		// Creating the database catalog which consists of details about all the tables
		// and schema.
		DatabaseCatalog dbc = DatabaseCatalog.getInstance();

		// Loading the database catalog with the contents present in samples/db folder
		// (in this case) or any folder specified during runtime.
	...
	/**
	 * JSQLParser method. Reads SQL statement from a string and stores them in
	 * separate variables
	 * 
	 * @param fileName   The name of the file that contains the SQL query to be
	 *                   parsed
	...
	public static void parseQuery(String fileName, String outputFile) {
		try {
			...
			if (statement != null) {
				...
				// Checking for null is essential as getGroupByExpressionList raises exception
				// when getGroupBy returns null
				if (select.getPlainSelect().getGroupBy() == null)
					GROUPBY = null;
				else
					GROUPBY = select.getPlainSelect().getGroupBy().getGroupByExpressionList();

				// Creating a root and passing all the parsed clauses to construct the root
				// operator.
			...
			// Catch block to hold any exception that occurred during the parsing of the
			// input SQL file
